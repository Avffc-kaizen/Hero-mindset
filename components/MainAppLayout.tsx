...
// FIX: Corrected import path for utils to match project structure.
...
export const MainAppLayout: React.FC = () => {
  ...
  const sidebarNavLinks = [
    ...
    { to: "/app/mapa", icon: Map, label: "Mapa 360" },
  ...
  const SidebarContent = ({ onLinkClick }: { onLinkClick?: () => void }) => (
     <div className="flex flex-col h-full bg-zinc-950">
        ...
        <NavLink to="/app/profile" onClick={onLinkClick} className="p-4 flex items-center gap-3 border-b border-zinc-800 hover:bg-zinc-900">
    ...
    <div className="flex h-screen bg-black overflow-hidden">
      ...
      {isMobileSidebarOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden">
          <div className="fixed inset-0 bg-black/80" onClick={() => setIsMobileSidebarOpen(false)}></div>
          <div className="relative w-64 bg-zinc-950 border-r border-zinc-800">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="flex-shrink-0 bg-zinc-950/80 backdrop-blur-sm border-b border-zinc-800 px-4 py-3 flex items-center gap-4">
            ...
            <div className="flex-grow flex items-center gap-4">
              <div className="font-mono text-sm"><span className="font-bold text-white">LVL {user.level}</span></div>
              <div className="flex-grow bg-zinc-800 rounded-full h-2.5"><div className="bg-gradient-to-r from-red-600 to-red-400 h-full rounded-full" style={{ width: `${xpProgress}%` }}></div></div>
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-6 bg-zinc-950">
          <Outlet />
        </div>
      </main>
    </div>
