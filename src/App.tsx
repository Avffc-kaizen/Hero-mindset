    ...
    <div className="flex h-screen bg-black overflow-hidden">
      ...
      {isMobileSidebarOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <div className="relative flex-1 flex flex-col max-w-xs w-full bg-zinc-950 border-r border-zinc-800">
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-16 bg-zinc-950 border-t border-zinc-800 md:hidden flex justify-around items-center z-40">
        <NavLink to="/app/dashboard" className={({isActive})=>`flex flex-col items-center gap-1 text-xs ${isActive?'text-white':'text-zinc-400'}`}><Compass/>Dashboard</NavLink>
        <NavLink to="/app/missions" className={({isActive})=>`flex flex-col items-center gap-1 text-xs ${isActive?'text-white':'text-zinc-400'}`}><Target/>Missões</NavLink>
        <NavLink to="/app/codex" className={({isActive})=>`flex flex-col items-center gap-1 text-xs ${isActive?'text-white':'text-zinc-400'}`}><Book/>Codex</NavLink>
        <NavLink to="/app/guild" className={({isActive})=>`flex flex-col items-center gap-1 text-xs ${isActive?'text-white':'text-zinc-400'}`}><Shield/>Guilda</NavLink>
        <button onClick={() => setIsMobileSidebarOpen(true)} className="flex flex-col items-center gap-1 text-xs text-zinc-400"><Menu/>Menu</button>
      </nav>
    </div>
  );
};
