...
import { Award, Zap, RefreshCw, Star, AlertCircle, Flame, Shield, Briefcase, Activity, TrendingUp, CheckCircle, Plus, Lock, Dumbbell, Moon, Droplets, Trash2, Bot, Target } from 'lucide-react';
                ...
                {items.length === 0 ? (
                    ...
                    items.map(item => (
                        <div key={item.id} className={`p-2 rounded border flex items-center gap-3 group/item ${item.completed ? 'bg-yellow-900/10 border-yellow-900/20 opacity-60' : 'bg-black/40 border-white/5 hover:border-yellow-500/30'}`}>
                            <div onClick={() => toggle(item.id)} className={`w-4 h-4 rounded-full border flex items-center justify-center cursor-pointer ${item.completed ? 'bg-yellow-600 border-yellow-600' : 'border-zinc-600'}`}>
                                {item.completed && <CheckCircle className="w-3 h-3 text-black" />}
                            </div>
                            <span className={`text-[10px] font-mono flex-grow ${item.completed ? 'line-through text-zinc-500' : 'text-zinc-200'}`}>{item.title}</span>
                             <button onClick={() => remove(item.id)} className="opacity-0 group-hover/item:opacity-50"><Trash2 className="w-3 h-3 text-red-500"/></button>
                        </div>
                    ))
                )}
            </div>
...
const BioShieldWidget = () => {
    ...
    const handleUpdate = (field: keyof BioData, increment: number) => handleUpdateUser({ bioData: { ...bioData, [field]: Math.max(0, (bioData[field] || 0) + increment) }});
    ...
    const StatButton: React.FC<{field: keyof BioData, value: number, label: string, icon: React.ElementType, color: string, increment: number}> = ({field, value, label, icon: Icon, color, increment}) => (
        <div className="bg-black/40 p-3 rounded border border-red-900/20 text-center group relative">
            ...
            <div className="absolute inset-0 flex justify-between items-center px-1 opacity-0 group-hover:opacity-100">
                 <button onClick={() => handleUpdate(field, -increment)} className="w-6 h-6 rounded-full bg-black/50">-</button>
                 <button onClick={() => handleUpdate(field, +increment)} className="w-6 h-6 rounded-full bg-black/50">+</button>
            </div>
        </div>
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-zinc-900/40 border border-white/10 rounded-2xl p-6 flex flex-col justify-between backdrop-blur-md">
            ...
            <div>
                ...
                <div className="w-full bg-black/50 rounded-full h-1.5 border border-white/5"><div className={`h-full rounded-full bg-red-600`} style={{ width: `${xpProgress}%` }}></div></div>
