...
const Pantheon: React.FC<PantheonProps> = ({ user, onSpendPoint }) => {
  // Verifica se o usuário já ascendeu. Se não, mostra tela de bloqueio.
  if (!user.isAscended) {
    return (
      <div className="p-6 max-w-4xl mx-auto text-center h-full flex flex-col items-center justify-center">
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-8 sm:p-12 max-w-2xl relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center">
                ...
                <h2 className="text-3xl font-black font-mono uppercase text-zinc-200 mb-4 tracking-tighter">
                    Acesso Restrito
                </h2>
                
                <p className="text-zinc-400 text-lg mb-8 leading-relaxed max-w-md">
                    O Panteão é o salão dos imortais. Apenas heróis que atingiram o ápice do potencial humano (Nível 50) e escolheram a <span className="text-white font-bold">Ascensão</span> podem entrar aqui.
                </p>
                ...
                <div className="flex flex-col sm:flex-row gap-4 text-sm font-mono text-zinc-500 bg-zinc-950 p-4 rounded-lg border border-zinc-800">
                    <div className="flex items-center gap-2">
                        <span className={user.level >= 50 ? "text-green-500" : "text-zinc-600"}>●</span>
                        Requisito: Nível 50
                    </div>
                    <div className="hidden sm:block text-zinc-700">|</div>
                    <div className="flex items-center gap-2">
                        <span className={user.isAscended ? "text-green-500" : "text-zinc-600"}>●</span>
                        Status: Ascendido
                    </div>
                </div>
            </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 border-b border-zinc-800 pb-6">
        <div>
            ...
            <p className="text-zinc-400 mt-2 font-mono text-sm">
                A força adquirida aqui ecoa por todas as suas vidas futuras.
            </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {PARAGON_PERKS.map((perk) => {
          ...
          return (
            <div 
                key={perk.id} 
                className="group bg-zinc-900 border border-zinc-800 hover:border-yellow-500/50 rounded-xl p-6 flex flex-col transition-all duration-300 hover:shadow-lg hover:shadow-yellow-900/10 relative overflow-hidden"
            >
              {/* Background Progress Tint */}
              <div 
                className="absolute bottom-0 left-0 top-0 bg-yellow-500/5 transition-all duration-500 pointer-events-none" 
              ...
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-4">
                        <div className={`w-14 h-14 rounded-xl flex items-center justify-center transition-colors ${isMaxLevel ? 'bg-yellow-500 text-black shadow-lg shadow-yellow-500/20' : 'bg-zinc-800 text-zinc-400 group-hover:text-yellow-500'}`}>
                        ...
                        <div>
                            ...
                            <p className="text-xs text-zinc-500 font-mono uppercase mt-1">
                                Nível <span className="text-white font-bold">{currentLevel}</span> <span className="text-zinc-700">/</span> {perk.maxLevel}
                            </p>
                        </div>
                    </div>
                ...
                <div className="min-h-[60px] mb-4">
                    ...
                    {!isMaxLevel && (
                        <div className="flex items-center gap-2 mt-2 text-xs text-zinc-500">
                            ...
                            <span>Próximo: <span className="text-green-400">{perk.description(currentLevel + 1)}</span></span>
                        </div>
                    )}
                </div>
                
                {/* Progress Bar */}
                <div className="w-full bg-zinc-950 rounded-full h-1.5 mb-6 border border-zinc-800/50">
                    <div 
                        className={`h-1.5 rounded-full transition-all duration-500 ${isMaxLevel ? 'bg-yellow-500' : 'bg-zinc-600 group-hover:bg-yellow-600'}`} 
                        style={{ width: `${progressPercent}%` }}
                    ></div>
                </div>
                ...
                <button
                    ...
                    className={`w-full py-3 font-bold uppercase tracking-widest rounded-lg transition-all active:scale-95 flex items-center justify-center gap-2 text-xs
                        ${isMaxLevel 
                ...
                >
                    ...
                    <>
                        ...
                        {canAfford ? 'Ascender Atributo' : 'Pontos Insuficientes'} 
                        {!isMaxLevel && <span className="ml-1 opacity-80">({cost} pts)</span>}
                    </>
                    )}
                </button>
              </div>
