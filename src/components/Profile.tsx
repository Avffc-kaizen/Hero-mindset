...
const Profile: React.FC<ProfileProps> = ({ user, onUpdateProfile, onDeleteAccount, onUpgrade, onPasswordChange }) => {
  ...
  // Keep local tempName in sync if parent user.name changes
  useEffect(() => {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setPasswordMessage(result?.message || (result.success ? 'Solicitação enviada.' : 'Falha ao solicitar redefinição.'));
    } catch (err) {
  ...
  const handleDeleteConfirm = () => {
    if (deleteConfirmation === user.name) {
      // Close modal immediately and call delete handler.
  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex-grow">
            {isEditingName ? (
              <div className="flex items-center gap-2">
                ...
                />
                <button onClick={handleSaveName} aria-label="Salvar nome" className="p-2 bg-zinc-700 hover:bg-zinc-600 rounded transition active:scale-95">
                  <Save className="w-5 h-5 text-white" />
                </button>
                <button onClick={() => { setIsEditingName(false); setTempName(user.name); }} aria-label="Cancelar edição" className="p-2 bg-zinc-800 hover:bg-zinc-700 rounded transition active:scale-95">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setIsEditingName(true)} aria-label="Editar nome" className="text-zinc-500 hover:text-white transition-colors active:scale-95">
      ...
      <div className="bg-red-950/20 border-2 border-red-900/50 rounded-xl p-6 space-y-3">
        ...
        <div className="flex gap-6 pt-4 text-[10px] text-zinc-600 font-mono uppercase tracking-wider border-t border-red-900/30 mt-4">
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-zinc-900 border border-red-900 rounded-xl p-6 max-w-md w-full">
            ...
            <input
              ...
              aria-label="Confirme seu nome para deletar conta"
            />
            <div className="flex gap-4">
              ...
              <button
                ...
                aria-disabled={deleteConfirmation !== user.name}
